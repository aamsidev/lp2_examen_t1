...
import model.DentistaWendelEC;
import model.EquipoDentalWendelEC;
...
public class DlgEquipoDental extends JDialog implements ActionListener {
	...
	void cargarDentistas() {
	    ...
	    try {
	        List<Object> dentistas = manager.createQuery("select d from DentistaWendelEC d", Object.class).getResultList();
	...
	void listar() {
	    ...
	    String jpql = "SELECT e, d.nombreCompleto FROM EquipoDentalWendelEC e LEFT JOIN e.dentista d ORDER BY e.nroEquipo";

	    try {
	       List<Object[]> resultados = manager.createQuery(jpql, Object[].class).getResultList();

	        for (Object[] fila : resultados) {
	            EquipoDentalWendelEC equipo = (EquipoDentalWendelEC) fila[0];
	            String dentistaNombre = (String) fila[1];

	            imprimir("--------------------------------------------------");
	            imprimir("Nro. Equipo.......: " + equipo.getNroEquipo());
	            imprimir("Nombre............: " + equipo.getNombre());
	            imprimir("Costo.............: " + equipo.getCosto());
	            imprimir("Fecha Adquisición.: " + equipo.getFechAdq());
	            imprimir("Estado............: " + equipo.getEstado());
	            imprimir("Dentista..........: " + (dentistaNombre != null ? dentistaNombre : "Sin asignar"));
	        }
	    } finally {
	        manager.close();
	    }
	...
	void adicionar() {
	    ...
	    DentistaWendelEC dentista = (DentistaWendelEC) cboDentistas.getSelectedItem();
	    LocalDate fechAdq = LocalDate.now();

	    EquipoDentalWendelEC equipo = new EquipoDentalWendelEC();
	    equipo.setNombre(nombre);
	    equipo.setCosto(costo);
	    equipo.setFechAdq(fechAdq);
	...
	void buscar() {
	    ...
	    try {
	        EquipoDentalWendelEC equipo = manager.find(EquipoDentalWendelEC.class, nroEquipo);
	        ...
	        txtFechaAdquisicion.setText(equipo.getFechAdq().toString());
